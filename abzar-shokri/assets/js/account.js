...
(function() {
  ...
  function renderWishlist(el) {
    ...
    el.innerHTML += '<div class="product-listing-grid" id="wishlist-grid">' + products.map(function(p) {
      ...
      return '<div class="product-card"><div class="product-card__image"><img src="' + p.image + '" alt="' + p.name + '" loading="lazy" class="product-card__img"></div><div class="product-card__body"><span class="product-card__brand">' + p.brandName + '</span><a href="product.html?slug=' + p.slug + '" class="product-card__title" style="color:inherit;text-decoration:none">' + p.name + '</a><div class="product-card__rating"><span class="product-card__stars">' + s + '</span><span>(' + getReviewCount(p.id) + ')</span></div><div class="product-card__price"><span class="product-card__price-current">' + Format.price(p.price) + ' <span class="product-card__price-currency">تومان</span></span></div><div style="display:flex;gap:var(--space-2)"><button class="btn btn-primary btn-sm" onclick="App.addToCart(' + p.id + ')" style="flex:1">افزودن به سبد</button><button class="btn btn-ghost btn-icon btn-sm" onclick="App.toggleWishlist(' + p.id + ');AccountPage.refresh()" style="color:var(--color-danger)">' + Icons.trash + '</button></div></div></div>';
    }).join('') + '</div>';
  }
