...
(function() {
  ...
  function render() {
    ...
    var stars = ''; for(var i=0;i<r.full;i++) stars += Icons.star; for(var i=0;i<r.empty;i++) stars += Icons.starEmpty;
    var badges = [];
    ...
    <div class="container"><div class="product-detail"><div class="product-detail__main">\
      <div class="product-gallery">\
        <div class="product-gallery__main"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1"><line x1="16.5" y1="9.4" x2="7.5" y2="4.21"/><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg></div>\
        <div class="product-gallery__thumbs">\
          <div class="product-gallery__thumb is-active"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/></svg></div>\
          <div class="product-gallery__thumb"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/></svg></div>\
          <div class="product-gallery__thumb"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/></svg></div>\
          <div class="product-gallery__thumb"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/></svg></div>\
        </div>\
      </div>\
    ...
    renderRelated();
    var qty = 1;
    window.ProductDetail = { changeQty: function(d) { qty = Math.max(1, qty + d); var el = $('#product-qty'); if(el) el.textContent = Format.toPersianNumber(qty); }, getQty: function() { return qty; } };
  }
  ...
  function renderReviews() {
    ...
    return reviews.map(function(rv) {
      var s = ''; for(var i=0;i<rv.rating;i++) s += Icons.star;
  ...
  function renderRelated() {
    ...
    grid.innerHTML = related.map(function(p) {
      ...
      return '<div class="product-card"><div class="product-card__image"><div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:var(--text-muted)"><span style="width:48px;height:48px">'+Icons.package+'</span></div>'+(p.discount>0?'<div class="product-card__badges"><span class="badge badge-discount">٪'+p.discount+'</span></div>':'')+'</div><div class="product-card__body"><span class="product-card__brand">'+p.brandName+'</span><a href="product.html?slug='+p.slug+'" class="product-card__title" style="color:inherit;text-decoration:none">'+p.name+'</a><div class="product-card__rating"><span class="product-card__stars">'+s+'</span><span>('+p.reviewCount+')</span></div><div class="product-card__price"><span class="product-card__price-current">'+Format.price(p.price)+' <span class="product-card__price-currency">تومان</span></span>'+(p.oldPrice?'<span class="product-card__price-old">'+Format.price(p.oldPrice)+'</span>':'')+'</div></div></div>';
    }).join('');
  }
